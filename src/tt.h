...
struct Entry
{
    ...
    constexpr int value(int ply) const
    {
        if (score != VALUE_NONE)
        {
    ...
    constexpr bool was_pv() const
    {
        return (about >> 9) & 1;
    }
    constexpr int generation() const
    {
        return (about >> 10);
    }
    constexpr void refresh(const int gen)
    {
        about = (about & 1023u) | (gen << 10u);
    }

    constexpr int generation_diff(const int tt_generation) const
    {
...
class HashTable
{
  ...
  public:
    ...
    constexpr uint64_t mul_hi(const uint64_t a, const uint64_t b) const
    {
        using uint128_t = unsigned __int128;
        return (static_cast<uint128_t>(a) * static_cast<uint128_t>(b)) >> 64;
    }

    void prefetch(const Key hash)
    {
    ...
    Entry *probe(const Key hash, bool &ttHit)
    {
        const uint64_t ind = mul_hi(hash, buckets);
    ...
    void save(Entry *entry, Key hash, int score, int depth, int ply, int bound, Move move, int eval, bool was_pv)
    {
        if (score != VALUE_NONE)
        {
            if (score >= TB_WIN_SCORE)
...
#ifndef GENERATE
std::unique_ptr<HashTable> TT; // shared hash table
#endif