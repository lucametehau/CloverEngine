...
int SearchThread::quiesce(int alpha, int beta, StackEntry* stack) {
    ...
    if (in_check) {
        ...
        stack->eval = best = eval = m_histories.get_corrected_eval(raw_eval, turn, m_board.pawn_key(), m_board.mat_key(WHITE), m_board.mat_key(BLACK), stack);
        futility_base = best + QuiesceFutilityBias;
    }
    else { /// ttValue might be a better evaluation
        raw_eval = eval;
        stack->eval = eval = m_histories.get_corrected_eval(raw_eval, turn, m_board.pawn_key(), m_board.mat_key(WHITE), m_board.mat_key(BLACK), stack);
...
int SearchThread::search(int alpha, int beta, int depth, StackEntry* stack) {
    ...
    if (in_check) { /// when in check, don't evaluate
        ...
        if (stack->excluded) raw_eval = eval = stack->eval;
            ...
            raw_eval = evaluate(m_board, NN);
            stack->eval = eval = m_histories.get_corrected_eval(raw_eval, turn, pawn_key, white_mat_key, black_mat_key, stack);
    ...
    while ((move = picker.get_next_move(m_histories, stack, m_board, skip, false)) != NULLMOVE) {
        ...
        if constexpr (!rootNode) {
            ...
            if (best > -MATE && m_board.has_non_pawn_material(turn)) {
                if (is_quiet) {
                    ...
                    int new_depth = std::max(0, depth - lmr_red[std::min(63, depth)][std::min(63, played)] / 1024 + improving + history / MoveloopHistDiv);
                    ...
                    if (depth <= FPNoisyDepth && !in_check && 
                        static_eval + noisy_futility_margin(depth + is_ttmove_noisy, m_board.get_captured_type(move)) <= alpha) continue;
                }
            }
        }
        ...
        if constexpr (rootNode) {
            ...
            if (main_thread() && printStats && m_info.get_time_elapsed() > 2500 && !m_info.is_san_mode()) {
                std::cout << "info depth " << depth << " currmove " << move.to_string(m_info.is_chess960()) << " currmovenumber " << played << std::endl;
            }
        }

        int new_depth = depth + ex, R = 1024;
        ...
        if (depth >= 2 && played > 1 + pvNode + rootNode) { // first few moves we don't reduce
            if (is_quiet) {
                ...
                R += LMRGoodEval * (enemy_has_no_threats && !in_check && eval - seeVal[PieceTypes::KNIGHT] > beta); // if the position is relatively quiet and eval is bigger than beta by a margin
                R += LMREvalDifference * (enemy_has_no_threats && !in_check && static_eval - m_root_eval > EvalDifferenceReductionMargin && ply % 2 == 0); /// the position in quiet and static eval is way bigger than root eval
                ...
                R -= 1024 * history / HistReductionDiv; // reduce based on move history
            }
            else if (!was_pv) {
                R = lmr_red[std::min(63, depth)][std::min(63, played)];
                R += LMRNoisyNotImproving * (improving <= 0); // not improving
                R += LMRBadNoisy * (enemy_has_no_threats && picker.trueStage == Stages::STAGE_BAD_NOISY); // if the position is relatively quiet and the capture is "very losing"
                R -= 1024 * history / CapHistReductionDiv; // reduce based on move history
            }
            ...
            R += LMRBadStaticEval * (enemy_has_no_threats && !in_check && static_eval + LMRBadStaticEvalMargin <= alpha);
            R -= 1024 * std::abs(raw_eval - static_eval) / LMRCorrectionDivisor;
    ...
    if (main_thread()) {
        for (int i = 1; i < 64; i++) { /// depth
            for (int j = 1; j < 64; j++) { /// moves played 
                lmr_red[i][j] = 1024 * (LMRQuietBias + log(i) * log(j) / LMRQuietDiv);
            }
        }
    }
#endif
