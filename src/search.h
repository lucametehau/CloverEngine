...
int SearchThread::search(int alpha, int beta, int depth, StackEntry *stack)
{
    ...
    while ((move = picker.get_next_move(m_histories, stack, m_board, skip, false)) != NULLMOVE)
    {
        ...
        { // first few moves we don't reduce
            ...
            //                         picker.trueStage == Stages::STAGE_BAD_NOISY); // if the position is relatively
            //                         quiet
            ...
            if (R > 1 && score > alpha)
            {
                ...
                //      ","
                //      << eval << "," << static_eval - m_root_eval << "," << ttDepth << "," << (ttValue <= alpha &&
                //      ttHit)
                //      << "," << history << "," << alpha << "," << beta << "," << (m_board.checkers() != 0) << ","
                //      << pvNode << "," << rootNode << "," << cutNode << ",";
                // fout << 1 << "\n";
