...
public:
    ...
    uint64_t key, pawn_key, mat_key[2], major_key;
    Threats threats;
...
public:
    ...
    void move_from_to(Square from, Square to) {
        ...
        if (pt == PieceTypes::PAWN) pawn_key() ^= hash_delta;
            ...
            if (!(pt == PieceTypes::KNIGHT || pt == PieceTypes::BISHOP)) major_key() ^= hash_delta;
        }
    ...
    void place_piece_at_sq(Piece piece, Square sq) {
        ...
        if (piece.type() == PieceTypes::PAWN) pawn_key() ^= hashKey[piece][sq];
            ...
            if (!(piece.type() == PieceTypes::KNIGHT || piece.type() == PieceTypes::BISHOP)) major_key() ^= hashKey[piece][sq];
        }
    ...
    void erase_square(Square sq) {
        ...
        if (pt == PieceTypes::PAWN) pawn_key() ^= hashKey[piece][sq];
            ...
            if (!(pt == PieceTypes::KNIGHT || pt == PieceTypes::BISHOP)) {
