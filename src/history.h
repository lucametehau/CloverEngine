...
class Histories
{
  private:
    ...
    MultiArray<CorrectionHistory, CorrectionHistory::SIZE, 2> minor_corr_hist;
    MultiArray<CorrectionHistory, CorrectionHistory::SIZE, 2> major_corr_hist;
  ...
  public:
    void clear_history()
    {
        ...
        fill_multiarray<CorrectionHistory, CorrectionHistory::SIZE, 2>(minor_corr_hist, CorrectionHistory(0));
        fill_multiarray<CorrectionHistory, CorrectionHistory::SIZE, 2>(major_corr_hist, CorrectionHistory(0));
    }
    ...
    CorrectionHistory &get_minor_corr_hist(const bool turn, const Key minor_key)
    {
        return minor_corr_hist[minor_key & CorrectionHistory::MASK][turn];
    }

    constexpr CorrectionHistory get_minor_corr_hist(const bool turn, const Key minor_key) const
    {
        return minor_corr_hist[minor_key & CorrectionHistory::MASK][turn];
    }
    ...
    {
        return major_corr_hist[major_key & CorrectionHistory::MASK][turn];
    }

    constexpr CorrectionHistory get_major_corr_hist(const bool turn, const Key major_key) const
    {
        return major_corr_hist[major_key & CorrectionHistory::MASK][turn];
    }
    ...
    void update_corr_hist(const bool turn, const Key pawn_key, const Key white_mat_key, const Key black_mat_key,
                          const Key minor_key, const Key major_key, StackEntry *stack, const int depth, const int delta)
    {
    ...
    const int get_corrected_eval(const int eval, const bool turn, const Key pawn_key, const Key white_mat_key,
                                 const Key black_mat_key, const Key minor_key, const Key major_key,
                                 StackEntry *stack) const
    {
        int correction =
            CorrHistPawn * get_corr_hist(turn, pawn_key) + CorrHistMat * get_mat_corr_hist(turn, WHITE, white_mat_key) +
            CorrHistMat * get_mat_corr_hist(turn, BLACK, black_mat_key) +
            CorrHistMinor * get_minor_corr_hist(turn, minor_key) + CorrHistMajor * get_major_corr_hist(turn, major_key);
        if ((stack - 1)->move)
        {
            if ((stack - 2)->move)
