...
public:
    ...
    void process_move(uint16_t move, Piece piece, Piece captured, int king, bool side, int16_t* a, int16_t* b) {
        int posFrom = sq_from(move), posTo = sq_to(move);
        bool turn = color_of(piece);
        switch (type(move)) {
        case NEUT: {
            if (captured == NO_PIECE)
                apply_sub_add(a, b, net_index(piece, posFrom, king, side), net_index(piece, posTo, king, side));
            else
                apply_sub_add_sub(a, b, net_index(piece, posFrom, king, side), net_index(piece, posTo, king, side), net_index(captured, posTo, king, side));
        }
        break;
        case CASTLE: {
            ...
            apply_sub_add_sub_add(a, b, net_index(piece, posFrom, king, side), net_index(piece, posTo, king, side), net_index(rPiece, rFrom, king, side), net_index(rPiece, rTo, king, side));
        }
        break;
        case ENPASSANT: {
            const int pos = sq_dir(turn, SOUTH, posTo);
            const Piece pieceCap = get_piece(PAWN, 1 ^ turn);
            apply_sub_add_sub(a, b, net_index(piece, posFrom, king, side), net_index(piece, posTo, king, side), net_index(pieceCap, pos, king, side));
        }
        break;
        default: {
            const int promPiece = get_piece(promoted(move) + KNIGHT, turn);
            if (captured == NO_PIECE)
                apply_sub_add(a, b, net_index(piece, posFrom, king, side), net_index(promPiece, posTo, king, side));
            else
                apply_sub_add_sub(a, b, net_index(piece, posFrom, king, side), net_index(promPiece, posTo, king, side), net_index(captured, posTo, king, side));
        }
        break;
        }
    }
